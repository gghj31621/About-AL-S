...
async function Draw(){
  ...
  if(!t){
    ...
    sd = 3
    for(i=sd;i--;){
      X = S(p=Math.PI*2/sd*i + Math.PI/sd) /1
      Y = C(p) / 1
      Z = 0
      a = [...a, [X,Y,Z]]
    }
    base_shp = [...base_shp, a]
    tgtDepth = 4
    recurseFronds = (tx, ty, tz, depth, theta, px, py, pz) => {
      if(depth>tgtDepth){
        P = [...P, [tx,ty,tz,(ct++%2)?0:0]]
        return
      }
      let a = JSON.parse(JSON.stringify(base_shp))
      let ls = 1/(1+depth*1.75)
      a.map((v,i)=>{
        v.map(q => {
         X = q[0] * ls
         Y = q[1] * ls
         Z = q[2] * ls
         let p_ = Math.atan2(px-tx,py-ty)/(8+C(t*20)*2)
         R(theta-p_,0,0)
         q[0] = X += tx
         q[1] = Y += ty
         q[2] = Z += tz
         
         recurseFronds(X, Y, Z, depth+1, theta-p_, tx, ty, tz)
        })
      })
      fronds = [...fronds, ...a]
    }
    genFronds = () => {
      P = []
      fronds = [], ct=0
      recurseFronds(0, .33, 0, 0, 0, 0, .33, 0)
      tfronds = JSON.parse(JSON.stringify(fronds))
      tP = JSON.parse(JSON.stringify(P))
      for(m_=3;m_--;){
        fronds = [...fronds, ...JSON.parse(JSON.stringify(tfronds)).map(v=>{
          v.map(q=>{
            X = q[0]
            Y = q[1]
            Z = q[2]
        ...
        P = [...P, ...JSON.parse(JSON.stringify(tP)).map(v=>{
          X = v[0]
          Y = v[1]
          Z = v[2]
          R(0,0,Math.PI/3*m_)
  ...
  oX=0, oY=0, oZ=Math.min(3,Math.max(1.5,(.3+C(t/2))*10))
  Rl=C(t)/4, Pt=-.4-C(t)/3, Yw=S(t/4)*4-t/2
  ...
  if(starsLoaded) P.map((v, i)=>{
    ...
    if(Z>0){
      l = Q()
      s = Math.min(1e3, 150/Z * (.6+C(t*20+i)/2))
      x.drawImage(starImgs[v[3]].img,l[0]-s/2/1.05,l[1]-s/2/1.05,s,s)
    }
  })
